...
* @Description: MyBatis 将mybatis要执行的sql拦截打印出来
 * logback已经实现了这个功能，所以不用这个了
 * 暂时保留着以作参考
* @author duanzhiwei
* @date 2018年1月18日 下午1:08:33  
*
 */
@Intercepts
({
...
public class SqlPrintInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        configuration.getEnvironment().getDataSource().getConnection().getMetaData().getURL();
        long end = System.currentTimeMillis();
        long timing = end - start;
        if(logger.isInfoEnabled()){
            DatabaseMetaData databaseMetaData = configuration.getEnvironment().getDataSource().getConnection().getMetaData();
            logger.info("使用数据库：" + databaseMetaData.getURL());
